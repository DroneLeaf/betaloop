        ...
        def _window_score(token: str) -> int:
            ...
            try:
                info = subprocess.run(
                    ...
                    capture_output=True,
                    text=True,
                    timeout=1,  # Prevent hanging on broken windows
                )
...
def main():
    ...
    if args.display:
        bridge_cmd += ["--display"]
        log.info("SDL2 direct display mode enabled")
    if args.shm:
        bridge_cmd += ["--shm"]
    ...
    if args.display:
        print(f"  Mode         : DISPLAY (SDL2 direct, lowest latency)")
    elif args.raw:
    ...
    if output_mode == "display":
        print("  FPV: rendered in SDL2 window by gz_image_bridge (no encoding)")
